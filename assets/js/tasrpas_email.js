(function($){
	$.fn.extend( {
		tasrpas_is_email: function( value ) {
			return /^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?$/i.test( this.val() );
		}
	} );
	$( document ).on( 'ready', TasrpasEmail_do );
	$( document.body ).on( 'post-load', TasrpasEmail_do );

	function TasrpasEmail_do() {


		$( 'a.tas-email-form' ).each( function() {
			if ( $( this ).attr( 'href' ) && $( this ).attr( 'href' ).indexOf( 'tas_message=' ) != -1 )
		...
		$( '.email-link' ).each( function( item ) {
			...
			//var $showform = $(this).closest("div");
			//if ($showform.find('#tessa_authorship_email_form').length == 0)
			//	$('#tessa_authorship_email_form').appendTo($showform);
			/*
			$('#tessa_authorship_email_form')
				.css('width', '90%')
				.css('border', '1px solid #999')
				.css('background-color', 'rgba(220,220,220,0.9)')
				.css('padding', '15px 15px 15px 15px')
				.css('border-radius', '8px')
				.css('position', 'absolute')
				.css('z-index', '20000000')
				.css('left', '50%')
				.css('top', '50%')
				.css('transform', 'translate(-45%,-50%)');
			*/
			$('#tessa_authorship_email_form .tas_form_buttons')
				.css('width', '300px')
				.css('margin', '0 auto');
			$('#tasrpas_recaptcha')
				.css('width', '300px')
				.css('margin','0 auto');
			// Email button
			$( 'a.tas-email-form', this ).on( 'click', function() {
				var url = $( this ).attr( 'href' ), key;

				if ( $( '#tessa_authorship_email_form' ).is( ':visible' ) )
					$( '#tessa_authorship_email_form' ).slideUp( 200 );
				else {

					$( '#tessa_authorship_email_form .response' ).remove();
					$( '#tessa_authorship_email_form form' ).show();

					$( '#tessa_authorship_email_form form input[type=submit]' ).removeAttr( 'disabled' );
					$( '#tessa_authorship_email_form form a.sharing_cancel' ).show();

					key = '';
					if ( $( '#recaptcha_public_key' ).length > 0 )
						key = $( '#recaptcha_public_key' ).val();
					// Update the recaptcha
					Recaptcha.create( key, 'tasrpas_recaptcha', { lang : recaptcha_options.lang } );

					// Show dialog
					$( '#tessa_authorship_email_form' ).css( {
						left: $( this ).offset().left + 'px',
						top: $( this ).offset().top + $( this ).height() + 'px'
					} ).slideDown( 200 );

					// Hook up other buttons
					$( '#tessa_authorship_email_form a.tas_cancel' ).unbind( 'click' ).click( function() {
						$( '#tessa_authorship_email_form .errors' ).hide();
						$( '#tessa_authorship_email_form' ).slideUp( 200 );
						return false;
					} );
					// Hook up other buttons
					$( '#tas_close_btn' ).unbind( 'click' ).click( function() {
						$( '#tessa_authorship_email_form .errors' ).hide();
						$( '#tessa_authorship_email_form' ).slideUp( 200 );
						return false;
					} );
					...
					// Submit validation
					$( '#tessa_authorship_email_form input[type=submit]' ).unbind( 'click' ).click( function() {
						var form = $( this ).parents( 'form' );

						// Disable buttons + enable loading icon
						$( this ).prop( 'disabled', true );
						form.find( 'a.tas_cancel' ).hide();
						form.find( 'img.loading' ).show();

						$( '#tessa_authorship_email_form .errors' ).hide();
						$( '#tessa_authorship_email_form .error' ).removeClass( 'error' );

						if ( $( '#tessa_authorship_email_form input[name=source_email]' ).tasrpas_is_email() == false )
							$( '#tessa_authorship_email_form input[name=source_email]' ).addClass( 'error' );

						if ( $( '#tessa_authorship_email_form .error' ).length == 0 ) {
							// AJAX send the form
							$.ajax( {
								url: url,
								type: 'POST',
								...
								success: function( response ) {
									...
									if ( response == '1' || response == '2' || response == '3' || response == '4' || response == '5' || response == '8' ) {
										$( '#tessa_authorship_email_form .errors-' + response ).show();
										form.find( 'input[type=submit]' ).removeAttr( 'disabled' );
										form.find( 'a.tas_cancel' ).show();
										Recaptcha.reload();
									}
									else {
										$( '#tessa_authorship_email_form form' ).hide();
										$( '#tessa_authorship_email_form' ).append( response );
										$( '#tessa_authorship_email_form a.tas_cancel' ).click( function() {
											$( '#tessa_authorship_email_form' ).slideUp( 200 );
											return false;
										} );
									}
						...
						form.find( 'input[type=submit]' ).removeAttr( 'disabled' );
						form.find( 'a.tas_cancel' ).show();
						$( '#tessa_authorship_email_form .errors-1' ).show();
...
// Recaptcha code
// Check if already loaded by a JetPack implementation or a plugin
if ('undefined' == typeof RecaptchaTemplates) {
...
if(!d)d=document.body;var e=d.className;e=e.replace(RegExp("(^|\\s+)"+a+"(\\s+|$)")," ");e=e.replace(RegExp("(^|\\s+)"+b+"(\\s+|$)")," ");e+=" "+(c?a:b);d.className=e},_get_help_link:function(){var a=RecaptchaOptions.lang;return"http://www.google.com/recaptcha";},playAgain:function(){var a=Recaptcha.$;a("recaptcha_image").innerHTML=Recaptcha.getAudioCaptchaHtml()},getAudioCaptchaHtml:function(){var a=Recaptcha,b=RecaptchaState,c=b.server+"image?c="+b.challenge;if(c.indexOf("https://")==

<?php
/*
*PluginName: Multiple Post Thumbnails
*PluginURI: http://wordpress.org/extend/plugins/multiple-post-thumbnails/
*Description: Adds the ability to add multiple post thumbnails to a post type.
*Version: 1.6
*Author: Chris Scott
*Author URI: http://voceplatforms.com/
*/
/*  Note by Uli Hake (uli|dot|hake|at|gmail|dot|com)
	This version is slightly modified to allow loading from class directory of Tessa Authorship.

	If Tessa Authorship detects the presence of the Multiple Post Thumbnails plugin it will use the plugin version.

	In the settings page of Tessa Authorship the admin is informed about the plugin including the link to WordPress Org for convenience.
*/
...
if (!class_exists('MultiPostThumbnails')) {
	...
	class MultiPostThumbnails {
		...
		public function enqueue_admin_scripts( $hook ) {
			...
			if (version_compare($wp_version, '3.5', '<')) {	
				add_thickbox();
				wp_enqueue_script( "mpt-featured-image", plugins_url( 'assets/js/multi-post-thumbnails-admin.js', $this->base_file ), array( 'jquery', 'media-upload' ) );
			} else { // 3.5+ media modal
				wp_enqueue_media();
				wp_enqueue_script( "mpt-featured-image", plugins_url( 'assets/js/multi-post-thumbnails-admin.js', $this->base_file ), array( 'jquery', 'set-post-thumbnail' ) );
				wp_enqueue_script( "mpt-featured-image-modal", plugins_url( 'assets/js/media-modal.js', $this->base_file ), array( 'jquery', 'media-models' ) );				
			}
		}
